...
```{r load_data}
dnam_data <- readRDS(file.path(raw_dir, "DNAm_Data.RDS"))
```
...
for (plate in unique(dnam_data$Sample_Plate)) {
  print(paste0("detectionP on : ", plate))
  detp_df <- rbind(
    detp_df,
    detectionp_plate(dnam_data, plate, threshold = 0.01)
  )
}
```
...
dnam_data <- dnam_data[,!(dnam_data$IDATid %in% to_drop)]
na_df <- na_df %>%
