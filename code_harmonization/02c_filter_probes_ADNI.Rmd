---
title: "Read in ADNI data, get detection P scores, filter out unwanted probes, and convert to beta values"
author: 
...
```{r save}
saveRDS(dnam_data, file = file.path(data_dir, "DNAm_beta.RDS"))
write.csv(
  detp_df,
...
write.csv(
  na_df,
  file = file.path(data_dir, "probes_missing.csv"),
  row.names = FALSE
)
row_df <- dnam_data %>%
  SummarizedExperiment::rowData() %>%
...
write.csv(
  row_df,
  file = file.path(data_dir, "probes_filtered.csv"),
  row.names = TRUE
)
write.csv(
  pheno_df,
