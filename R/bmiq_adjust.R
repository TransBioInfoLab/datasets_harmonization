#' Adjustment to wateRmelon's BMIQ function. There is an error in taking the 
#' mean of 2 values, that causes the function to sometimes crash. This fixes
#' that error.
BMIQ_adjust <- function(
    beta.v,
    design.v,
    nL = 3,
    doH = TRUE,
  ...
  subsetth1.v <- c(
    mean(
      c(
        max(beta1.v[rand.idx[subsetclass1.v==1]]), min(beta1.v[rand.idx[subsetclass1.v==2]])
      )
    ),
    mean(
      c(
        max(beta1.v[rand.idx[subsetclass1.v==2]]), min(beta1.v[rand.idx[subsetclass1.v==3]])
      )
    )
  );
  ...
  subsetth2.v <- c(
    mean(
      c(
        max(beta2.v[rand.idx[subsetclass2.v==1]]),min(beta2.v[rand.idx[subsetclass2.v==2]])
      )
    ),
    mean(
      c(
        max(beta2.v[rand.idx[subsetclass2.v==2]]),min(beta2.v[rand.idx[subsetclass2.v==3]])
      )
    )
  );
...
CheckBMIQ_adjust <- function(beta.v,design.v,pnbeta.v){### pnbeta is BMIQ normalised profile
